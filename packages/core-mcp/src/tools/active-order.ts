 ...
 */
export function registerActiveOrderTools(server: McpServer) {
  server.tool(
    'activate_order',
    `Activa un pedido como foco del agente. DEBE llamarse siempre que empieces a trabajar sobre un pedido especifico, y cada vez que cambies de pedido.
...
Si cambias de pedido, vuelve a llamar activate_order con el nuevo id — reemplaza el anterior.`,
  ...
  server.tool(
    'deactivate_order',
    'Desactiva el pedido activo. El frontend sale del cockpit mode y vuelve al layout general. Usa esto cuando termines de trabajar sobre un pedido y quieras volver a la vista general.',
    {},
    async () => {
  ...
  server.tool(
    'get_active_order',
    'Devuelve el pedido activo actual (si hay). Util si no recuerdas sobre que pedido estas trabajando.',
    {},
    async () => {
